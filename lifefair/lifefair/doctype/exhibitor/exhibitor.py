...
def get_exhibitors(event=None):
    if event:
        sql_query = """SELECT 
             ...
             LEFT JOIN 
               (SELECT `parent`, `text`, `date` 
                FROM `tabPerson Quote` AS `t11`
                WHERE `t11`.`name` = (SELECT `name` 
                    FROM `tabPerson Quote` AS `t22`
                    WHERE `t22`.`parent` = `t11`.`parent`
                    ORDER BY `date` DESC LIMIT 1)) AS `tTestimonial2`
             ...
             LEFT JOIN 
               (SELECT `parent`, `text`, `date` 
                FROM `tabPerson Quote` AS `t111`
                WHERE `t111`.`name` = (SELECT `name` 
                    FROM `tabPerson Quote` AS `t222`
                    WHERE `t222`.`parent` = `t111`.`parent`
                    ORDER BY `date` DESC LIMIT 1)) AS `tTestimonial3`
